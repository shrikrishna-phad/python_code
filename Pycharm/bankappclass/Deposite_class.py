...
class Deposit:
    ...
    def deposit(self):
        ...
        while flag:
            try:
                self.amount = int(input("Enter the amount you want to Deposit (in indian rupees) : "))
            except ValueError:
        ...
        if not flag:
            ...
            logger.info(f"Customer with account number {self.data[self.data.index(self.ind)][7]} "
                        f"has deposited Rs.{self.bal} to the account\n "
                        f"new balance-{self.data[self.data.index(self.ind)][8]}")
